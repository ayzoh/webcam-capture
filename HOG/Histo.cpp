...
{
    // Supprimer le if pour la synth�se
...
void Histo::affiche_graphique(string nom, int pattern[N_CLASSES][CELL_SIZE*CELL_SIZE])
{
    ...
    printf("width: %d\n", m_width);
    printf("histo_width: %d\n", m_histo_width);
    printf("height: %d\n", m_height);
    printf("histo_height: %d\n", m_histo_height);

    fichier_out << "P2" << endl << m_width / (CELL_SIZE / 4) << " " << m_height / (CELL_SIZE / 4) - 2 << endl << "255" << endl;
    ...
    for(j = 0; j < m_histo_height; j++) {
        for(int occur = 0; occur < 4; occur++) {
            for(i = 0; i < m_histo_width; i++) {
                ...
                for(k = 0; k < CELL_SIZE / (CELL_SIZE / 4); k++) {
                    fichier_out << (int)round(val*pattern[classe][k+occur*(CELL_SIZE / (CELL_SIZE / 4))]/1024) << " ";
                }
            }
            fichier_out << endl;
        }
